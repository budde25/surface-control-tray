...
impl Device {
    ...
    pub fn open_path<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        match result {
            ...
            Err(e) => Err(e).context(ErrorKind::DeviceAccess).map_err(Into::into),
        }
    }

    pub fn latch_lock(&self) -> Result<()> {
        unsafe {
            dtx_latch_lock(self.file.as_raw_fd()).context(ErrorKind::Io)?;
        }
        Ok(())
    }
    ...
    pub fn latch_unlock(&self) -> Result<()> {
        unsafe {
            dtx_latch_unlock(self.file.as_raw_fd()).context(ErrorKind::Io)?;
        }
        Ok(())
    }
    ...
    pub fn latch_request(&self) -> Result<()> {
        unsafe {
            dtx_latch_request(self.file.as_raw_fd()).context(ErrorKind::Io)?;
        }
        Ok(())
    }
